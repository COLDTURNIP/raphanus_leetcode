...
testing sequence, and a sorted list releaseTimes, where releaseTimes[i] was the time the ith key
was released. Both arrays are 0-indexed. The 0th key was pressed at the time 0, and every
...
Example 1:
    ...
    Explanation: The keypresses were as follows:
        ...
        - Keypress for 'b' had a duration of 29 - 9 = 20 (pressed at time 9 right after the release
          of the previous character and released at time 29).
        - Keypress for 'c' had a duration of 49 - 29 = 20 (pressed at time 29 right after the
          release of the previous character and released at time 49).
        - Keypress for 'd' had a duration of 50 - 49 = 1 (pressed at time 49 right after the
          release of the previous character and released at time 50).
        - The longest of these was the keypress for 'b' and the second keypress for 'c', both with
          duration 20.
...
impl Solution {
    pub fn slowest_key(release_times: Vec<i32>, keys_pressed: String) -> char {
        ...
        for (ts, c) in release_times
            .into_iter()
            .zip(keys_pressed.into_bytes().into_iter())
        {
            let duration = ts - last_ts;
            if (duration, c).cmp(&(longest_duration, longest_char)) != std::cmp::Ordering::Less {
...
mod tests {
    ...
    fn bench(b: &mut test::Bencher) {
        b.iter(|| Solution::slowest_key(vec![12, 23, 36, 46, 62], "spuda".to_owned()));
    }
}
